export const guestTranslations = {
  title: {
    es: "🏠 Página para Huéspedes",
    en: "🏠 Guest Page",
    pt: "🏠 Página para Hóspedes"
  },
  subtitle: {
    es: "¡Gracias por alojarte en nuestra casa! Como cualquier casa, viene con mantenimiento. Aquí tienes un **libro de autoayuda** para lidiar con los problemas más comunes de una casa, *cualquier casa*. Por supuesto, siempre puedes contactarnos, pero a veces una solución fácil está a la vuelta de la esquina. 🔧",
    en: "Thank you for staying in our house! As any house, it comes with maintenance. Here's a **self-help book** to deal with the most common issues of a house, *any house*. Of course you can always contact us, but sometimes an easy fix is just around the corner. 🔧",
    pt: "Obrigado por se hospedar na nossa casa! Como qualquer casa, ela vem com manutenção. Aqui está um **livro de autoajuda** para lidar com os problemas mais comuns de uma casa, *qualquer casa*. Claro que você sempre pode nos contatar, mas às vezes uma solução fácil está logo ali. 🔧"
  },
  wifi: {
    title: {
      es: "📶 WiFi",
      en: "📶 WiFi",
      pt: "📶 WiFi"
    },
    content: {
      es: `📍 **Red WiFi:** *Il Buco*  
🔑 **Contraseña:** *terminator2*

🔍 **Si no encuentras la red:**
- Busca redes abiertas estándar de **TP-Link**
- Esto pasa cuando alguien reinicia el punto de acceso
- *dormitorio1* **no es nuestra red** ❌

🚫 **Si no hay internet:**
El problema probablemente sea con el **cable de fibra óptica**. Contacta al proveedor:

<div class="fibra-card">fibraalhogar.com.ar</div>

Han tenido mal servicio, lo siento 😕`,
      en: `📍 **WiFi Network:** *Il Buco*  
🔑 **Password:** *terminator2*

🔍 **If you can't find the network:**
- Look for **TP-Link** standard open networks
- That happens when someone restarts the access point
- *dormitorio1* **is not ours** ❌

🚫 **If there's no internet:**
The problem is most likely with the **fiber optic cable**. Contact the provider:

<div class="fibra-card">fibraalhogar.com.ar</div>

They had bad service, sorry 😕`,
      pt: `📍 **Rede WiFi:** *Il Buco*  
🔑 **Senha:** *terminator2*

🔍 **Se não conseguir encontrar a rede:**
- Procure por redes abertas padrão **TP-Link**
- Isso acontece quando alguém reinicia o ponto de acesso
- *dormitorio1* **não é nossa** ❌

🚫 **Se não houver internet:**
O problema provavelmente é com o **cabo de fibra óptica**. Entre em contato com o provedor:

<div class="fibra-card">fibraalhogar.com.ar</div>

Eles tiveram serviço ruim, desculpe 😕`
    }
  },
  electricity: {
    title: {
      es: "⚡ Electricidad",
      en: "⚡ Electricity",
      pt: "⚡ Eletricidade"
    },
    content: {
      es: `🏠 **Localización de equipos eléctricos:**
- Ve al **sótano** y gira a la derecha **dos veces**
- Todo lo eléctrico está en la **sala de máquinas**

🔌 **Verificar disyuntores:**
- Busca los **disyuntores caídos** 📋
- Intenta encenderlos
- ⚠️ **Si saltan de vuelta**, no lo intentes hasta encontrar el problema

🌧️ **Si está mojado afuera:**
- Probablemente hay **agua en la lámpara exterior**
- Apaga las **luces exteriores** 💡
- Intenta el disyuntor de nuevo

📞 **Contacto profesional:**

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Electricista</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>`,
      en: `🏠 **Location of electrical equipment:**
- Go to the **basement** and turn right **twice**
- Everything electrical is in the **machine room**

🔌 **Check breakers:**
- Look for **breakers that are down** 📋
- Try to turn them on
- ⚠️ **If they jump back**, don't try until you find the problem

🌧️ **If it's wet outside:**
- It's probably **water in the outdoor light fixture**
- Turn off the **outdoor lights** 💡
- Try the breaker again

📞 **Professional contact:**

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Electrician</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>`,
      pt: `🏠 **Localização dos equipamentos elétricos:**
- Vá ao **porão** e vire à direita **duas vezes**
- Tudo elétrico está na **sala de máquinas**

🔌 **Verificar disjuntores:**
- Procure por **disjuntores desligados** 📋
- Tente ligá-los
- ⚠️ **Se eles saltarem de volta**, não tente até encontrar o problema

🌧️ **Se estiver molhado lá fora:**
- Provavelmente há **água na luminária externa**
- Desligue as **luzes externas** 💡
- Tente o disjuntor novamente

📞 **Contato profissional:**

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Eletricista</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>`
    }
  },
  water: {
    title: {
      es: "💧 Agua",
      en: "💧 Water",
      pt: "💧 Água"
    },
    content: {
      es: `🚰 **Si NO hay agua:**
Probablemente sea un **problema eléctrico**. Contacta a David (electricista).

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Electricista</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>

🌊 **Si hay EXCESO de agua o mal olor:**
Es trabajo del **plomero**.

<div class="bg-white border-2 border-blue-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-blue-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">🔧</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">Plomeros Martinez</h3>
      <p class="text-gray-600 text-sm">Plomería</p>
      <a href="https://wa.me/542254440021" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 2254 44-0021
      </a>
    </div>
  </div>
</div>`,
      en: `🚰 **If there's NO water:**
That's probably an **electrical problem**. Contact David (electrician).

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Electrician</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>

🌊 **If there's EXCESS water or bad smell:**
It's the **plumber's job**.

<div class="bg-white border-2 border-blue-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-blue-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">🔧</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">Plomeros Martinez</h3>
      <p class="text-gray-600 text-sm">Plumbing</p>
      <a href="https://wa.me/542254440021" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 2254 44-0021
      </a>
    </div>
  </div>
</div>`,
      pt: `🚰 **Se NÃO houver água:**
Provavelmente é um **problema elétrico**. Entre em contato com David (eletricista).

<div class="bg-white border-2 border-yellow-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-yellow-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">⚡</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">David</h3>
      <p class="text-gray-600 text-sm">Eletricista</p>
      <a href="https://wa.me/541158783996" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 11 5878-3996
      </a>
    </div>
  </div>
</div>

🌊 **Se houver EXCESSO de água ou cheiro ruim:**
É trabalho do **encanador**.

<div class="bg-white border-2 border-blue-400 rounded-lg p-4 shadow-md max-w-sm mx-auto mt-4">
  <div class="flex items-center space-x-3">
    <div class="w-12 h-12 bg-blue-400 rounded-full flex items-center justify-center">
      <span class="text-2xl">🔧</span>
    </div>
    <div>
      <h3 class="font-bold text-gray-900 text-lg">Plomeros Martinez</h3>
      <p class="text-gray-600 text-sm">Encanamento</p>
      <a href="https://wa.me/542254440021" class="text-green-600 font-medium text-sm hover:underline">
        📱 +54 9 2254 44-0021
      </a>
    </div>
  </div>
</div>`
    }
  },
  passwordPrompt: {
    es: "🔐 Acceso Requerido",
    en: "🔐 Access Required",
    pt: "🔐 Acesso Requerido"
  },
  passwordPlaceholder: {
    es: "Ingresa la contraseña",
    en: "Enter password",
    pt: "Digite a senha"
  },
  accessButton: {
    es: "🚪 Acceder",
    en: "🚪 Access",
    pt: "🚪 Acessar"
  },
  incorrectPassword: {
    es: "❌ Contraseña incorrecta",
    en: "❌ Incorrect password",
    pt: "❌ Senha incorreta"
  }
}